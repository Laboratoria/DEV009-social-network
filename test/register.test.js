...
jest.mock('../lib/firebase', () => ({
  createUser: jest.fn((userEmail, userPassword) => {
    if (userEmail === 'valid@example.com' && userPassword === 'validPassword') {
      return Promise.resolve({        // usuario con info correcta
        uid: 'usuario-id',
  ...
  signInWithGoogle: jest.fn(() => {     // usuario con info erronea
...
describe('register.js', () => {
  it('debe registrar usuario con mail y clave válida', async () => {
    ...
    const navigateTo = jest.fn(); // jest.fn rastrear si llama a la función con los parametros entregados
